define(['datetime', 'imageLoader', 'connectionManager', 'itemHelper', 'focusManager', 'indicators', 'globalize', 'layoutManager', 'apphost', 'dom', 'browser', 'emby-button', 'paper-icon-button-light', 'clearButtonStyle'],
        ...
        function buildCardsHtmlInternal(items, options) {
            ...
            for (var i = 0, length = items.length; i < length; i++) {
                ...
                if (options.indexBy) {
                    ...
                    else if (options.indexBy === 'DateCreated') {
                        if (item.DateCreated) {
                            try {

                                newIndexValue = datetime.toLocaleDateString(datetime.parseISO8601Date(item.DateCreated), { weekday: 'long', month: 'long', day: 'numeric' });
